<html><head><meta http-equiv="content-type" content="text/html; charset=utf-8" /></head><body><h3>Building a Better WCF Client</h3><p>The WCF client that Visual Studio generates by default is, to&nbsp;put it bluntly, a problem. Besides not being a direct match with the server-side interface code, which causes a lot of code and data sharing problems, the generated code still has a glaring bug after all these years. <a href="http://msmvps.com/blogs/p3net/default.aspx">Michael Taylor</a> discusses these issue and his attempts to address them in a series titled “<a href="http://msmvps.com/blogs/p3net/archive/2014/02/02/a-smarter-wcf-service-client-part-1.aspx">A Smarter WCF Service Client</a>”.</p>
...
<p>A common mistake when using a service reference style WCF client is the use of a 'using' block. While common, this practice is inappropriate because the WCF client may throw an exception when Dispose is invoked. This is known as the “<a href="http://www.theroks.com/wcf-dispose-problem-with-using-statement/">WCF Dispose Problem</a>” and work-arounds are quite common.</p>
...
<p>Michaels first offering is <a href="http://msmvps.com/blogs/p3net/archive/2014/02/09/a-smarter-wcf-service-client-part-2.aspx">a subclass of ClientBase&lt;T&gt; that correctly implements the IDisposable pattern</a>. He writes,</p>
<blockquote> 
 <p>Another interesting point about the wrapper is that it implements IDisposable. <a href="http://msdn.microsoft.com/en-us/library/ms576141(v=vs.110).aspx">ClientBase&lt;T&gt;</a> already implements this interface so why are we reimplementing it? Because the base type doesn’t follow the correct approaching to implementing the interface. When implementing this interface a class is supposed to provide an overridable method that derived classes can implement but this one does not. In order to fix the cleanup code we have to reimplement the interface and the actual method. But we will implement it properly, both in terms of derived types and in handling faulted channels.</p> 
</blockquote>
<p>In <a href="http://msmvps.com/blogs/p3net/archive/2014/02/23/a-smarter-wcf-service-client-part-3.aspx">part three</a> of his ongoing series, Michael talks about how to reduce the boilerplate needed for making individual service calls into a single line. For example,</p>
...
<p>Michael intends to correct these issues in a future article.</p><br><br><br><br><br><br></body></html>